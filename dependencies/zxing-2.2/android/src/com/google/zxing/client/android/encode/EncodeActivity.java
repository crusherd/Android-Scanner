...
public final class EncodeActivity extends Activity {
  ...
  public boolean onCreateOptionsMenu(final Menu menu) {
    ...
    final boolean useVcard = (qrCodeEncoder != null) && qrCodeEncoder.isUseVCard();
    final int encodeNameResource = useVcard ? R.string.menu_encode_mecard : R.string.menu_encode_vcard;
    final MenuItem encodeItem = menu.findItem(R.id.menu_encode);
    encodeItem.setTitle(encodeNameResource);
    final Intent intent = getIntent();
    if (intent != null) {
      final String type = intent.getStringExtra(Intents.Encode.TYPE);
  ...
  private void share() {
    final QRCodeEncoder encoder = qrCodeEncoder;
    if (encoder == null) { // Odd
      Log.w(TAG, "No existing barcode to send?");
      return;
    }

    final String contents = encoder.getContents();
    if (contents == null) {
      Log.w(TAG, "No existing barcode to send?");
      return;
    }
    ...
    final File bsRoot = new File(Environment.getExternalStorageDirectory(), "BarcodeScanner");
    final File barcodesRoot = new File(bsRoot, "Barcodes");
    if (!barcodesRoot.exists() && !barcodesRoot.mkdirs()) {
      Log.w(TAG, "Couldn't make dir " + barcodesRoot);
      showErrorMessage(R.string.msg_unmount_usb);
      return;
    }
    final File barcodeFile = new File(barcodesRoot, makeBarcodeFileName(contents) + ".png");
    barcodeFile.delete();
  ...
  private static CharSequence makeBarcodeFileName(final CharSequence contents) {
  ...
  protected void onResume() {
    ...
    try {
      final boolean useVCard = intent.getBooleanExtra(USE_VCARD_KEY, false);
      ...
      final ImageView view = (ImageView) findViewById(R.id.image_view);
      view.setImageBitmap(bitmap);

      final TextView contents = (TextView) findViewById(R.id.contents_text_view);
